...
import static com.hunofox.pictureselector.widget.pictureEditWeidgt.DrawUtil.GRAFFITI_PIXEL_UNIT;
import static com.hunofox.pictureselector.widget.pictureEditWeidgt.DrawUtil.computeAngle;
import static com.hunofox.pictureselector.widget.pictureEditWeidgt.DrawUtil.drawCircle;
import static com.hunofox.pictureselector.widget.pictureEditWeidgt.DrawUtil.drawLine;
import static com.hunofox.pictureselector.widget.pictureEditWeidgt.DrawUtil.drawRect;
...
public class GraffitiView extends View {
    ...
    public static final int MIN_PATH = 10;
...
*/
    ...
    public GraffitiView(Context context,
                        Bitmap bitmap,
                        String eraser,
                        boolean eraserImageIsResizeable,
                        GraffitiListener listener) {
        super(context);
    ...
    public void init() {
        ...
        mAmplifierPaint.setStrokeWidth(dp2px(getContext(), 10));
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                // 仅单点时的操作
                if (Math.abs(mTouchDownX - mTouchX) <= MIN_PATH
                        && Math.abs(mTouchDownY - mTouchY) <= MIN_PATH
                        && Math.abs(mTouchDownX - mLastTouchX) <= MIN_PATH
                        && Math.abs(mTouchDownY - mLastTouchY) <= MIN_PATH) {
                    if (!isItemChecked()) {
                        if(mSelectedItem != null){// 取消选定
                            ...
                            if (mGraffitiListener != null) {
                                mGraffitiListener.onSingleClick();
                            }
                        }
                    }
                    return false;
                } else if (Math.abs(mTouchDownX - mTouchX) <= MIN_PATH && Math.abs(mTouchDownY - mTouchY) <= MIN_PATH) {
                    return false;
                }
            ...
            case MotionEvent.ACTION_MOVE:
                if (mTouchMode < 2) { // 单点滑动
                    ...
                    if (!(Math.abs(mTouchDownX - mTouchX) > MIN_PATH || Math.abs(mTouchDownY - mTouchY) > MIN_PATH)) {
                        return true;
                    }
    ...
    private void setBG() {// 不用resize preview
        ...
        DrawUtil.setGraffitiPixelUnit(dp2px(getContext(), 1) / mPrivateScale);
...
//            }
    ...
    public void addWaterMark(String text, float textSize) {
        GraffitiText graffitiText = new GraffitiText(
                Pen.TEXT,
                text,
                textSize,
                getColor(),
                0,
                getGraffitiRotateDegree(),
                getOriginalPivotX() * 2 - textSize * (text.length() * 0.5f),//X
                getOriginalPivotY() * 1.95f,//Y
                0,
                0);

        draw(mBitmapCanvas, graffitiText);
        invalidate();
    }
