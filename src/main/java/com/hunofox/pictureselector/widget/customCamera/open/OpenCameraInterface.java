...
public final class OpenCameraInterface {
    ...
    /**
     * For {@link #open(int, CameraFacing, int)}, means no preference for which camera to open.
     */
     ...
     */
    public static OpenCamera open(int cameraId, CameraFacing facing, int degrees) {
        ...
        if (facing == CameraFacing.FRONT) {
            result = (selectedCameraInfo.orientation + degrees) % 360;
            result = (360 - result) % 360; // compensate the mirror
        } else {
            result = (selectedCameraInfo.orientation - degrees + 360) % 360;
        }
